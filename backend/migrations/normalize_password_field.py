...
class MigratePasswordField:
    ...
    async def apply(db: AsyncIOMotorDatabase):
        ...
        try:
            ...
            await migrations_collection.insert_one({
                "name": MigratePasswordField.name,
            ...
            print(f"  ✅ {MigratePasswordField.name} completada")
            return True
            
        except Exception as e:
            print(f"  ❌ Error en {MigratePasswordField.name}: {str(e)}")
            raise
    ...
    async def rollback(db: AsyncIOMotorDatabase):
        ...
        try:
            print(f"[{datetime.utcnow().isoformat()}] Revirtiendo {MigratePasswordField.name}...")
            ...
            migrations_collection = db.migrations_log
            await migrations_collection.insert_one({
                "name": f"{MigratePasswordField.name}_rollback",
                "applied_at": datetime.utcnow(),
                "status": "rolled_back"
            })
            
            print(f"  ✅ {MigratePasswordField.name} revertida")
            return True
            
        except Exception as e:
            print(f"  ❌ Error revirtiendo {MigratePasswordField.name}: {str(e)}")
            raise
